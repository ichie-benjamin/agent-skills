...
When the user asks to **validate, lint, inspect, score, or scene-critic an existing composition** — rather than produce a new film — skip the production flow entirely: no Intake, no pre-flight, no brand KB. Works from any cwd; the only dependencies are this skill's `references/scene-critic-rubric.md` and the HF CLI.
...
| `references/script-judge-rubric.md` | this skill | Writing-quality gate at the Plan gate — VO + captions, fresh agent |
...
| Scene design depth (Flow step 4) | full | full | full | light | light | – |
...
Master per `research/patterns/pattern-premium-vo.md`: -11 to -14 LUFS, true peak ≤ 0, LRA ≤ 3, one intentional peak, one quiet beat, clean silent stop. **Verify the master objectively** with `audio-sanity-check.sh <master> --profile master` — it checks the master actually hit that loudness/true-peak/LRA/clean-stop spec (a tool fact, not a feel judgment; premium feel stays the ear at Gate 1).
